describe('Radio Input test Suite', () => {
  beforeEach(() => {
    cy.visit('/radio-fields'); // "baseUrl" is defined in cypress.json file
  });

  context('General', () => {
    it('should change input value when type', () => {
      cy.get('input[type="radio"][value="crash-bandicoot"]').check().blur();
      cy.get('input[type="radio"][value="crash-bandicoot"]').should(
        'be.checked',
      );
    });
    it('should be disabled when prop disable is true', () => {
      cy.get('input[type="radio"][value="disable"]').should('be.disabled');
    });
  });
    ...
    it('should update FormValues when input changes', () => {
      ...
      cy.get('[data-formValues]')
        ...
        .should(formValues => {
          ...
          expect(parsed.character).to.be.equal('crash-bandicoot');
        });
    });
  });
  ...
  context('Required Fields', () => {
    it('should have an "*" on label if field is required', () => {
      cy.get('legend[for="required"]').should('contain', '*');
    });
  });
  ...
  context('Changes & Submit', () => {
    it('should not complain about non-required empty fields if form is submitted', () => {
      ...
      cy.get('input[name="console"]')
        .parent()
        .parent()
        .should('not.have.class', 'radio-group--error');
    });
    ...
    it('should complain about empty required fields if form is submitted', () => {
      ...
      cy.get('input[name="required"]')
        .parent()
        .parent()
        .should('have.class', 'radio-group--error');
    });
